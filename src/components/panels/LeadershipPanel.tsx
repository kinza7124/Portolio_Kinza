    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-3xl font-serif gold-text">Leadership</h2>
        </div>
          ...
          >
            ...
            <div className="space-y-2 p-3 rounded-lg bg-secondary/20 border border-border/15 hover:border-primary/20 transition-colors">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <h3 className="font-serif font-semibold text-foreground text-sm">{item.role}</h3>
                  <p className="text-xs text-primary/80 font-medium">{item.organization}</p>
                  <p className="text-[11px] text-muted-foreground/70">{item.period}</p>
                </div>
                {item.link && (
              ...
              <div className="flex flex-wrap gap-1">
                {item.tags.map((tag, tagIndex) => (
                  <Badge key={tagIndex} variant="outline" className="text-[10px] bg-primary/5 text-primary/60 border-primary/10">
                    {tag}
                  </Badge>
                ))}
              </div>
